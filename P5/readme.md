## Practica 5: Reporte de usabilidad 

## Caso A

### Participantes

En la tabla a continuación se muestra información acerca de los usuarios que realizarán las pruebas:

La prueba para comprobar la usabilidad del sitio consiste en que los distintos usuarios intentarán realizar un pedido en la página, en este caso, pedirán una hamburguesa kevin bacon.

| id_participante | edad | genero | competencia digital | gafas/lentillas | Rol |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 06 | 20 | Masculino | Alta | No | Estudiante |
| 07 | 50 | Masculino | Baja | Si | Comercial |
| 09 | 23 | Femenino | Media | Sí | Estudiante |
| 010 | 20 | Masculino | Alta | Sí | Estudiante |

---

### **Usuario 06**

![.](<./info_usuarios/u6/1.jpg>)
![.](<./info_usuarios/u6/2.jpg>)

| PREGUNTAS | 1 | 2 | 3 | 4 | 5 |
| :--- | :---: | :---: | :---: | :---: | :---: |
| Creo que me gustará visitar con frecuencia este website | | | | | ✓ |
| Encontré el website innecesariamente complejo | | ✓ | | | |
| Pensé que era fácil utilizar este website | | | | | ✓ |
| Creo que necesitaría del apoyo de un experto para recorrer el website | ✓ | | | | |
| Encontré las funciones del website bastante bien integradas | | | | | ✓ |
| Pensé que había demasiada inconsistencia en el website | | ✓ | | | |
| Imagino que la mayoría de las personas aprenderían muy rápidamente a utilizar el website | | | | | ✓ |
| Encontré el website muy grande al recorrerlo | | | | ✓ | |
| Me sentí muy confiado en el manejo del website | | | | | ✓ |
| Necesito aprender muchas cosas antes de manejarse en el website | ✓ | | | | |

---

### **Usuario 07**

![.](<./info_usuarios/u7/1.jpg>)
![.](<./info_usuarios/u7/2.jpg>)

| PREGUNTAS | 1 | 2 | 3 | 4 | 5 |
| :--- | :---: | :---: | :---: | :---: | :---: |
| Creo que me gustará visitar con frecuencia este website | | | | ✓ |  |
| Encontré el website innecesariamente complejo | | ✓ | | | |
| Pensé que era fácil utilizar este website | | | | ✓ | |
| Creo que necesitaría del apoyo de un experto para recorrer el website | ✓ | | | | |
| Encontré las funciones del website bastante bien integradas | | | | | ✓ |
| Pensé que había demasiada inconsistencia en el website | ✓ | | | | |
| Imagino que la mayoría de las personas aprenderían muy rápidamente a utilizar el website | | | ✓ | | |
| Encontré el website muy grande al recorrerlo | | | | ✓ | |
| Me sentí muy confiado en el manejo del website | | | | ✓ | |
| Necesito aprender muchas cosas antes de manejarse en el website | ✓ | | | | |

---

### **Usuario 08**

![.](<./info_usuarios/u8/1.jpg>)
![.](<./info_usuarios/u8/2.jpg>)

| PREGUNTAS | 1 | 2 | 3 | 4 | 5 |
| :--- | :---: | :---: | :---: | :---: | :---: |
| Creo que me gustará visitar con frecuencia este website | | | ✓ | | |
| Encontré el website innecesariamente complejo | ✓ | | | | |
| Pensé que era fácil utilizar este website | | | | | ✓ |
| Creo que necesitaría del apoyo de un experto para recorrer el website | | ✓ | | | |
| Encontré las funciones del website bastante bien integradas | | | | ✓ | |
| Pensé que había demasiada inconsistencia en el website | ✓ | | | | |
| Imagino que la mayoría de las personas aprenderían muy rápidamente a utilizar el website | | | ✓ | | |
| Encontré el website muy grande al recorrerlo | | ✓ | | | |
| Me sentí muy confiado en el manejo del website | | | | ✓ | |
| Necesito aprender muchas cosas antes de manejarse en el website | | ✓ | | | |

**Usuario 09**

![.](<./info_usuarios/u9/1.jpg>)
![.](<./info_usuarios/u9/2.jpg>)

| PREGUNTAS | 1 | 2 | 3 | 4 | 5 |
| :--- | :---: | :---: | :---: | :---: | :---: |
| Creo que me gustará visitar con frecuencia este website | | | | ✓ | |
| Encontré el website innecesariamente complejo | | ✓ | | | |
| Pensé que era fácil utilizar este website | | | | ✓ | |
| Creo que necesitaría del apoyo de un experto para recorrer el website | ✓ | | | | |
| Encontré las funciones del website bastante bien integradas | | | ✓ | | |
| Pensé que había demasiada inconsistencia en el website | ✓ | | | | |
| Imagino que la mayoría de las personas aprenderían muy rápidamente a utilizar el website | | | | | ✓ |
| Encontré el website muy grande al recorrerlo |  | | ✓ | | |
| Me sentí muy confiado en el manejo del website | | | | | ✓ |
| Necesito aprender muchas cosas antes de manejarse en el website | ✓ | | | | |

**Usuario 010**

![.](<./info_usuarios/u10/1.jpg>)
![.](<./info_usuarios/u10/2.jpg>)

| PREGUNTAS | 1 | 2 | 3 | 4 | 5 |
| :--- | :---: | :---: | :---: | :---: | :---: |
| Creo que me gustará visitar con frecuencia este website | | | | ✓ | |
| Encontré el website innecesariamente complejo | ✓ | | | | |
| Pensé que era fácil utilizar este website | | | | | ✓ |
| Creo que necesitaría del apoyo de un experto para recorrer el website | ✓ | | | | |
| Encontré las funciones del website bastante bien integradas | | | ✓ | | |
| Pensé que había demasiada inconsistencia en el website | ✓ | | | | |
| Imagino que la mayoría de las personas aprenderían muy rápidamente a utilizar el website | | | | | ✓ |
| Encontré el website muy grande al recorrerlo | ✓ | | | | |
| Me sentí muy confiado en el manejo del website | | | | | ✓ |
| Necesito aprender muchas cosas antes de manejarse en el website | ✓ | | | | |

---

### **Desglose de Puntuaciones por Participante (Caso A)**

* **Usuario 06:**
  * *Impares:* P1(5->4), P3(5->4), P5(5->4), P7(5->4), P9(5->4) = **20**
  * *Pares:* P2(2->3), P4(1->4), P6(2->3), P8(4->1), P10(1->4) = **15**
  * *Cálculo:* (20 + 15) * 2.5 = **87.5**

* **Usuario 07:**
  * *Impares:* P1(4->3), P3(4->3), P5(5->4), P7(3->2), P9(4->3) = **15**
  * *Pares:* P2(2->3), P4(1->4), P6(1->4), P8(4->1), P10(1->4) = **16**
  * *Cálculo:* (15 + 16) * 2.5 = **77.5**

* **Usuario 08:**
  * *Impares:* P1(3->2), P3(5->4), P5(4->3), P7(3->2), P9(4->3) = **14**
  * *Pares:* P2(1->4), P4(2->3), P6(1->4), P8(2->3), P10(2->3) = **17**
  * *Cálculo:* (14 + 17) * 2.5 = **77.5**

* **Usuario 09:**
  * *Impares:* P1(4->3), P3(4->3), P5(3->2), P7(5->4), P9(5->4) = **16**
  * *Pares:* P2(2->3), P4(1->4), P6(1->4), P8(3->2), P10(1->4) = **17**
  * *Cálculo:* (16 + 17) * 2.5 = **82.5**

* **Usuario 010:**
  * *Impares:* P1(4->3), P3(5->4), P5(3->2), P7(5->4), P9(5->4) = **17**
  * *Pares:* P2(1->4), P4(1->4), P6(1->4), P8(1->4), P10(1->4) = **20**
  * *Cálculo:* (17 + 20) * 2.5 = **92.5**

---

### **Tabla Resumen de Resultados (Caso A)**

| id_participante | Total Impares | Total Pares | Suma Bruta | Puntuación SUS Final |
| :--- | :---: | :---: | :---: | :---: |
| **06** | 20 | 15 | 35 | **87.5** |
| **07** | 15 | 16 | 31 | **77.5** |
| **08** | 14 | 17 | 31 | **77.5** |
| **09** | 16 | 17 | 33 | **82.5** |
| **010** | 17 | 20 | 37 | **92.5** |
| **Promedio Global** | | | | **83.50** |

- Aceptabilidad: **Aceptable**
- Calificación: **A**

##Conclusiones del caso A

Podemos observar como la mirada de los usuarios se va hacia el header y la carta, lo que correspondia con nuestra propuesta de valor, además también se ha prestado bastante importancia a la barra de los filtros. Al igual que el caso B, el footer es casi completamente ignorado, siendo la última parte interesante para el usuario promedio la galería. En cuanto a la página de pedidos, las miradas se centran en el header, en el formulario de pedido y en los productos que se van a pedir, al tener la carta en el landing page el usuario se ahorra tener que ir a una página exclusiva de la carta, lo cual consideramos es una ventaja respecto al caso B.


---



## Caso B

### Participantes

En la tabla a continuación se muestra información acerca de los usuarios que realizarán las pruebas:

La prueba para comprobar la usabilidad del sitio consiste en que los distintos usuarios intentarán realizar un pedido en la página.

| id_participante | edad | genero | competencia digital | gafas/lentillas | Rol |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 01 | 20 | Masculino | Alta | No | Estudiante |
| 02 | 21 | Masculino | Alta | No | Estudiante |
| 03 | 46 | Femenino | Media | Sí | Ama de casa |
| 04 | 20 | Masculino | Alta | Sí | Natural | Estudiante |
| 05 | 48 | Femenino | Baja | No  | Profesora |

---

### **Usuario 01**

![.](<./info_usuarios/u1/1.jpg>)
![.](<./info_usuarios/u1/2.jpg>)
![.](<./info_usuarios/u1/3.jpg>)

| PREGUNTAS | 1 | 2 | 3 | 4 | 5 |
| :--- | :---: | :---: | :---: | :---: | :---: |
| Creo que me gustará visitar con frecuencia este website | | | ✓ | |  |
| Encontré el website innecesariamente complejo | | ✓ | | | |
| Pensé que era fácil utilizar este website | | | | | ✓ |
| Creo que necesitaría del apoyo de un experto para recorrer el website | ✓ | | | | |
| Encontré las funciones del website bastante bien integradas | | | | ✓ | |
| Pensé que había demasiada inconsistencia en el website | | ✓ | | | |
| Imagino que la mayoría de las personas aprenderían muy rápidamente a utilizar el website | | | | ✓ | |
| Encontré el website muy grande al recorrerlo | ✓ | | | | |
| Me sentí muy confiado en el manejo del website | | | | ✓ | |
| Necesito aprender muchas cosas antes de manejarse en el website | ✓ | | | | |

---

### **Usuario 02**

![.](<./info_usuarios/u2/1.jpg>)
![.](<./info_usuarios/u2/2.jpg>)
![.](<./info_usuarios/u2/3.jpg>)

| PREGUNTAS | 1 | 2 | 3 | 4 | 5 |
| :--- | :---: | :---: | :---: | :---: | :---: |
| Creo que me gustará visitar con frecuencia este website | | ✓ | | |  |
| Encontré el website innecesariamente complejo | | ✓ | | | |
| Pensé que era fácil utilizar este website | | | | ✓ | |
| Creo que necesitaría del apoyo de un experto para recorrer el website | ✓ | | | | |
| Encontré las funciones del website bastante bien integradas | | | | | ✓ |
| Pensé que había demasiada inconsistencia en el website | | | | ✓ | |
| Imagino que la mayoría de las personas aprenderían muy rápidamente a utilizar el website | | | | ✓ | |
| Encontré el website muy grande al recorrerlo | ✓ | | | | |
| Me sentí muy confiado en el manejo del website | | | | | ✓ |
| Necesito aprender muchas cosas antes de manejarse en el website | ✓ | | | | |

---

### **Usuario 03**

![.](<./info_usuarios/u3/1.jpg>)
![.](<./info_usuarios/u3/2.jpg>)
![.](<./info_usuarios/u3/3.jpg>)

| PREGUNTAS | 1 | 2 | 3 | 4 | 5 |
| :--- | :---: | :---: | :---: | :---: | :---: |
| Creo que me gustará visitar con frecuencia este website | | | | ✓ | |
| Encontré el website innecesariamente complejo | | | ✓ | | |
| Pensé que era fácil utilizar este website | | | | ✓ | |
| Creo que necesitaría del apoyo de un experto para recorrer el website | | ✓ | | | |
| Encontré las funciones del website bastante bien integradas | | | | ✓ | |
| Pensé que había demasiada inconsistencia en el website | | ✓ | | | |
| Imagino que la mayoría de las personas aprenderían muy rápidamente a utilizar el website | | | ✓ | | |
| Encontré el website muy grande al recorrerlo | | ✓ | | | |
| Me sentí muy confiado en el manejo del website | | | | ✓ | |
| Necesito aprender muchas cosas antes de manejarse en el website | ✓ | | | | |

**Usuario 04**

![.](<./info_usuarios/u4/1.jpg>)
![.](<./info_usuarios/u4/2.jpg>)

| PREGUNTAS | 1 | 2 | 3 | 4 | 5 |
| :--- | :---: | :---: | :---: | :---: | :---: |
| Creo que me gustará visitar con frecuencia este website | | | ✓ | | |
| Encontré el website innecesariamente complejo | ✓ | | | | |
| Pensé que era fácil utilizar este website | | | | | ✓ |
| Creo que necesitaría del apoyo de un experto para recorrer el website | ✓ | | | | |
| Encontré las funciones del website bastante bien integradas | | | | | ✓ |
| Pensé que había demasiada inconsistencia en el website | ✓ | | | | |
| Imagino que la mayoría de las personas aprenderían muy rápidamente a utilizar el website | | | | | ✓ |
| Encontré el website muy grande al recorrerlo | ✓ | | | | |
| Me sentí muy confiado en el manejo del website | | | | | ✓ |
| Necesito aprender muchas cosas antes de manejarse en el website | ✓ | | | | |

**Usuario 05**

![.](<./info_usuarios/u5/1.jpg>)
![.](<./info_usuarios/u5/2.jpg>)
![.](<./info_usuarios/u5/3.jpg>)

| PREGUNTAS | 1 | 2 | 3 | 4 | 5 |
| :--- | :---: | :---: | :---: | :---: | :---: |
| Creo que me gustará visitar con frecuencia este website | | | | ✓ | |
| Encontré el website innecesariamente complejo | | | ✓ | | |
| Pensé que era fácil utilizar este website | | | | ✓ | |
| Creo que necesitaría del apoyo de un experto para recorrer el website | | ✓ | | | |
| Encontré las funciones del website bastante bien integradas | | | | | ✓ |
| Pensé que había demasiada inconsistencia en el website | ✓ | | | | |
| Imagino que la mayoría de las personas aprenderían muy rápidamente a utilizar el website | | | | ✓|  |
| Encontré el website muy grande al recorrerlo | ✓ | | | | |
| Me sentí muy confiado en el manejo del website | | | | ✓ | |
| Necesito aprender muchas cosas antes de manejarse en el website | | ✓ | | | |

---

### **Desglose de Puntuaciones por Participante (Caso B)**

* **Usuario 01:**
  * *Impares:* P1(3->2), P3(5->4), P5(4->3), P7(4->3), P9(4->3) = **15**
  * *Pares:* P2(2->3), P4(1->4), P6(2->3), P8(1->4), P10(1->4) = **18**
  * *Cálculo:* (15 + 18) * 2.5 = **82.5**

* **Usuario 02:**
  * *Impares:* P1(2->1), P3(4->3), P5(5->4), P7(4->3), P9(5->4) = **15**
  * *Pares:* P2(2->3), P4(1->4), P6(4->1), P8(1->4), P10(1->4) = **16**
  * *Cálculo:* (15 + 16) * 2.5 = **77.5**

* **Usuario 03:**
  * *Impares:* P1(4->3), P3(4->3), P5(4->3), P7(3->2), P9(4->3) = **14**
  * *Pares:* P2(3->2), P4(2->3), P6(2->3), P8(2->3), P10(1->4) = **15**
  * *Cálculo:* (14 + 15) * 2.5 = **72.5**

* **Usuario 04:**
  * *Impares:* P1(3->2), P3(5->4), P5(5->4), P7(5->4), P9(5->4) = **18**
  * *Pares:* P2(1->4), P4(1->4), P6(1->4), P8(1->4), P10(1->4) = **20**
  * *Cálculo:* (18 + 20) * 2.5 = **95.0**

* **Usuario 05:**
  * *Impares:* P1(4->3), P3(4->3), P5(5->4), P7(4->3), P9(4->3) = **16**
  * *Pares:* P2(3->2), P4(2->3), P6(1->4), P8(1->4), P10(2->3) = **16**
  * *Cálculo:* (16 + 16) * 2.5 = **80.0**

---

### **Tabla Resumen de Resultados (Caso B)**

| id_participante | Total Impares | Total Pares | Suma Bruta | Puntuación SUS Final |
| :--- | :---: | :---: | :---: | :---: |
| **01** | 15 | 18 | 33 | **82.5** |
| **02** | 15 | 16 | 31 | **77.5** |
| **03** | 14 | 15 | 29 | **72.5** |
| **04** | 18 | 20 | 38 | **95.0** |
| **05** | 16 | 16 | 32 | **80.0** |
| **Promedio Global** | | | | **81.50** |

- Aceptabilidad: **Aceptable**
- Calificación: **A**

## Conclusiones del caso B

Observando los mapas de calor, podemos ver que en la landing page la interacción se concentra en su totalidad en la mitad superior de la página, siendo en el logo, en la navegación o en la parte central dependiendo del usuario. En el resto de páginas al igual que en la landing page, podemos observar que se le ha prestado nula atención al footer y al botón de contacto. En el caso de esta tarea llevada a cabo por los usuarios, podemos ver que la mayoria de los participantes han accedido a la carta, donde podemos observar que la concentración de las miradas se orienta a la navegación de la carta y a las imagenes de los productos. En el caso de la página de realizar el pedido, se concentran en el formulario de pedido, aunque algunos usuarios también han mirado al header.


# INFORME DE ACCESIBILIDAD WEB

## 1. Ficha Técnica del Informe
* **Nombre del proyecto:** GOIKΟ
* **Normativa de referencia:** WCAG 2.2 Nivel AA (UNE-EN 301549)
* **Herramientas utilizadas:** Lighthouse, WAVE y revisión manual
* **Fecha de la auditoría:** 31/05/2025

---

## 2. Puntuaciones Globales (Métricas Automáticas)
La evaluación se realizó utilizando herramientas automáticas de accesibilidad y revisión visual manual.

### Valoración General de Accesibilidad
El sitio presenta una estructura visual moderna y una navegación sencilla. Sin embargo, se identifican posibles incidencias relacionadas con contraste, accesibilidad semántica y navegación mediante teclado. El cumplimiento general puede considerarse parcial respecto a WCAG 2.2 Nivel AA.

---

## 3. Análisis por Principios (POUR)

### A. Perceptible
* **Hallazgo 1: Posibles problemas de contraste entre texto y fondo**
  * **Criterio WCAG incumplido:** 1.4.3 - Contraste mínimo.
  * **Impacto:** Los usuarios con baja visión pueden tener dificultades para distinguir determinados elementos visuales y textos.
  * **Recomendación:** Verificar todas las combinaciones de color mediante herramientas de contraste y asegurar una relación mínima de 4.5:1.

* **Hallazgo 2: Imágenes sin descripción alternativa verificable**
  * **Criterio WCAG incumplido:** 1.1.1 Contenido no textual.
  * **Impacto:** Los usuarios de lectores de pantalla podrían no comprender el contenido visual.
  * **Recomendación:** Añadir atributos `alt` descriptivos a las imágenes relevantes y marcar las decorativas con `alt=""`.

### B. Operable
* **Hallazgo 3: Navegación mediante teclado no completamente verificable**
  * **Criterio WCAG incumplido:** 2.1.1 - Teclado.
  * **Impacto:** Algunos usuarios podrían no poder acceder correctamente a elementos interactivos utilizando únicamente el teclado.
  * **Recomendación:** Comprobar que todos los enlaces, botones y componentes interactivos sean accesibles mediante Tab y Enter.

* **Hallazgo 4: Indicador de foco insuficiente o poco visible**
  * **Criterio WCAG incumplido:** 2.4.7 - Foco visible.
  * **Impacto:** Los usuarios pueden perder la referencia de navegación dentro de la página.
  * **Recomendación:** Aplicar estilos visuales claros para `:focus` y `:focus-visible`.

### C. Comprensible
* **Hallazgo 5: Tipografía decorativa con posible impacto en la legibilidad**
  * **Criterio WCAG relacionado:** 3.1 y 3.2 Comprensibilidad.
  * **Impacto:** Algunos usuarios con dificultades cognitivas o dislexia pueden requerir un mayor esfuerzo de lectura.
  * **Recomendación:** Mejorar tamaño, espaciado e interlineado de los textos.

* **Hallazgo 6: Estructura jerárquica de encabezados no verificable**
  * **Criterio WCAG incumplido:** 1.3.1 - Información y relaciones.
  * **Impacto:** Los lectores de pantalla podrían no interpretar correctamente la organización del contenido.
  * **Recomendación:** Utilizar encabezados semánticos (`h1`, `h2`, `h3`) siguiendo una jerarquía lógica.

### D. Robusto
* **Hallazgo 7: Etiquetas accesibles y roles ARIA no verificables**
  * **Criterio WCAG incumplido:** 4.1.2 Nombre, función y valor.
  * **Impacto:** Las tecnologías de asistencia pueden no identificar correctamente algunos elementos interactivos.
  * **Recomendación:** Revisar la implementación HTML y añadir atributos ARIA cuando sea necesario.

* **Hallazgo 8: Posible uso excesivo de contenedores genéricos**
  * **Criterio WCAG incumplido:** 1.3.1 y 4.1.2.
  * **Impacto:** Los lectores de pantalla pueden perder información estructural importante.
  * **Recomendación:** Utilizar etiquetas semánticas como `main`, `nav`, `header`, `section` y `footer`.

---

## 4. Tabla de Hallazgos y Prioridades

| ID | Prioridad | Criterio WCAG | Error detectado | Recomendación Técnica |
| :--- | :--- | :--- | :--- | :--- |
| **ACC-01** | Crítica | 1.4.3 Contraste mínimo | Contraste potencialmente insuficiente | Ajustar colores para alcanzar 4.5:1 |
| **ACC-02** | Alta | 1.1.1 Contenido no textual | Imágenes sin texto alternativo verificable | Añadir atributos alt descriptivos |
| **ACC-03** | Alta | 2.1.1 Teclado | Navegación por teclado no verificable | Garantizar acceso mediante teclado |
| **ACC-04** | Alta | 2.4.7 Foco visible | Indicador de foco poco visible | Implementar estilos visibles para: focus |
| **ACC-05** | Media | 1.3.1 Información y relaciones | Jerarquía de encabezados no verificable | Revisar estructura semántica |
| **ACC-06** | Media | Comprensibilidad | Tipografía con posible impacto en la lectura | Mejorar tamaño e interlineado |
| **ACC-07** | Media | 4.1.2 Nombre, función y valor | Roles accesibles no verificables | Revisar atributos ARIA |
| **ACC-08** | Baja | Semántica HTML | Uso potencial de contenedores genéricos | Implementar etiquetas semánticas |

---

## 5. Conclusiones y Declaración de Conformidad

### Declaración de conformidad
El sitio presenta un cumplimiento parcial de las WCAG 2.2 Nivel AA. Aunque visualmente resulta claro y atractivo, requiere mejoras relacionadas con contraste, navegación accesible, semántica HTML y compatibilidad con tecnologías de asistencia.

### ¿Es el sitio accesible?
Actualmente el sitio puede considerarse parcialmente accesible, pero no cumple completamente con los requisitos del nivel AA.

### Próximos pasos
1. Verificar y corregir problemas de contraste.
2. Añadir textos alternativos y etiquetas accesibles.
3. Mejorar la navegación mediante teclado y los indicadores de foco.

### Valoración final
* **Nivel estimado de accesibilidad:** 7/10
